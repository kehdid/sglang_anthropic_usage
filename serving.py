    ...
    async def _generate_anthropic_stream(
        ...
        async for sse_line in openai_stream:
                ...
                delta_event = AnthropicStreamEvent(
                    ...
                    usage=AnthropicUsage(
                        ...
                        cache_read_input_tokens=(
                            usage_info.get("cache_read_input_tokens") if usage_info else None
                        ),
                        output_tokens_details=(
                            AnthropicOutputTokensDetails(
                                reasoning_tokens=usage_info.get("reasoning_tokens")
                            )
                            if usage_info and usage_info.get("reasoning_tokens") is not None
                            else None
                        ),
                    ),
                )
            ...
            if getattr(chunk.usage, "prompt_tokens_details", None) is not None:
                cached_tokens = getattr(chunk.usage.prompt_tokens_details, "cached_tokens", None)
        ...
        return AnthropicMessagesResponse(
            ...
            usage=AnthropicUsage(
                ...
                cache_read_input_tokens=(
                    ...
                    if (
                        response.usage
                        and getattr(response.usage, "prompt_tokens_details", None) is not None
                    )
                    else None
                ),
                output_tokens_details=(
                    ...
                    if response.usage and getattr(response.usage, "reasoning_tokens", None) is not None
                    else None
                ),
            ),
        )
